...
class Random5Agent(Agent):
    ...
    class MonteCarloTreeSearchNode() :
        ...
        def expand(self):                               # Make next move and add as child
            ...
            child = Random5Agent.MonteCarloTreeSearchNode(
                board, next_move, self.p1_pos, self.max_steps, dir = dir, parent = self
            )
            
            num_bar = len(Random5Agent.allowed_barriers(next_move, self.chess_board))            # Heuristic to avoid trapping self in
            if (num_bar == 1) :
                child.Q += -50
            if self.state:
        ...
        def barrier_sims(self, move):
            ...
            for i in Random5Agent.allowed_barriers(move, self.chess_board):
                barrier_list[i] = 1
                total += 1
        ...
        def legal_moves(self, o_pos, adv_pos, chess_board):                                  # find all possible moves
            ...
            for i in range(self.max_steps):
                for pos in steps[i]:
                    moves = Random5Agent.allowed_dirs(pos, adv_pos, chess_board)  
        ...
        def random_moves(self, board):                                      # Literally random_agent
            ...
            while(len(barriers) == 1 and len(self.simulation_moves) > 0):
                ...
                barriers = Random5Agent.allowed_barriers(move, chess_board)
            return move
        ...
        def simulation_step(self, chess_board, my_pos, adv_pos):
            ...
            walls = Random5Agent.allowed_barriers(my_pos, chess_board)
